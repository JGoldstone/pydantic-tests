...
class Clip(CompatibleBaseModel):
    ...
    @classmethod
    def set_up_duration(cls):
        cls.add_property('duration', 'duration', ('static',))
        cls.add_property('camera_make', 'make', ('static', 'camera'))

# setattr(Clip, 'duration', property(lambda s: "foo", lambda s, v: None))
# Clip.add_property('duration', ('static',))
# Clip.set_up_duration()
Clip.setup_clip_properties()
# cm_clip = Clip.setup_clip_properties()
# print(f"cm_clip {'is' if cm_clip is Clip else 'is not'} Clip")
# foo=Clip()
# print(f"foo.camera_make is ...{foo.camera_make}...")
# foo.camera_make = 'arri'
print("pause here")
